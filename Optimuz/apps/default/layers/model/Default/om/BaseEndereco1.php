...
/**
 * Base class that represents a row from the 'endereco1' table.
 *
 * 
 *
 * @package    propel.generator.Default.om
 */
abstract class BaseEndereco1 extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'Endereco1Peer';
	 ...
	 * @return     Endereco1 The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = Endereco1Peer::ID;
		}
	...
	public function setCidadeId($v)
	{
		...
		if ($this->cidade_id !== $v) {
			$this->cidade_id = $v;
			$this->modifiedColumns[] = Endereco1Peer::CIDADE_ID;
		}
	...
	public function setLogradouro($v)
	{
		...
		if ($this->logradouro !== $v) {
			$this->logradouro = $v;
			$this->modifiedColumns[] = Endereco1Peer::LOGRADOURO;
		}
	...
	public function setBairro($v)
	{
		...
		if ($this->bairro !== $v) {
			$this->bairro = $v;
			$this->modifiedColumns[] = Endereco1Peer::BAIRRO;
		}
	...
	public function setCep($v)
	{
		...
		if ($this->cep !== $v) {
			$this->cep = $v;
			$this->modifiedColumns[] = Endereco1Peer::CEP;
		}
	...
	public function setNumero($v)
	{
		...
		if ($this->numero !== $v) {
			$this->numero = $v;
			$this->modifiedColumns[] = Endereco1Peer::NUMERO;
		}
	...
	public function setComplemento($v)
	{
		...
		if ($this->complemento !== $v) {
			$this->complemento = $v;
			$this->modifiedColumns[] = Endereco1Peer::COMPLEMENTO;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 7; // 7 = Endereco1Peer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating Endereco1 object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = Endereco1Peer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = Endereco1Query::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				Endereco1Peer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(Endereco1Peer::ID)) {
			$modifiedColumns[':p' . $index++]  = '`ID`';
		}
		if ($this->isColumnModified(Endereco1Peer::CIDADE_ID)) {
			$modifiedColumns[':p' . $index++]  = '`CIDADE_ID`';
		}
		if ($this->isColumnModified(Endereco1Peer::LOGRADOURO)) {
			$modifiedColumns[':p' . $index++]  = '`LOGRADOURO`';
		}
		if ($this->isColumnModified(Endereco1Peer::BAIRRO)) {
			$modifiedColumns[':p' . $index++]  = '`BAIRRO`';
		}
		if ($this->isColumnModified(Endereco1Peer::CEP)) {
			$modifiedColumns[':p' . $index++]  = '`CEP`';
		}
		if ($this->isColumnModified(Endereco1Peer::NUMERO)) {
			$modifiedColumns[':p' . $index++]  = '`NUMERO`';
		}
		if ($this->isColumnModified(Endereco1Peer::COMPLEMENTO)) {
			$modifiedColumns[':p' . $index++]  = '`COMPLEMENTO`';
		}

		$sql = sprintf(
			'INSERT INTO `endereco1` (%s) VALUES (%s)',
	...
	{
		$pos = Endereco1Peer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['Endereco1'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Endereco1'][$this->getPrimaryKey()] = true;
		$keys = Endereco1Peer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(Endereco1Peer::ID)) $criteria->add(Endereco1Peer::ID, $this->id);
		if ($this->isColumnModified(Endereco1Peer::CIDADE_ID)) $criteria->add(Endereco1Peer::CIDADE_ID, $this->cidade_id);
		if ($this->isColumnModified(Endereco1Peer::LOGRADOURO)) $criteria->add(Endereco1Peer::LOGRADOURO, $this->logradouro);
		if ($this->isColumnModified(Endereco1Peer::BAIRRO)) $criteria->add(Endereco1Peer::BAIRRO, $this->bairro);
		if ($this->isColumnModified(Endereco1Peer::CEP)) $criteria->add(Endereco1Peer::CEP, $this->cep);
		if ($this->isColumnModified(Endereco1Peer::NUMERO)) $criteria->add(Endereco1Peer::NUMERO, $this->numero);
		if ($this->isColumnModified(Endereco1Peer::COMPLEMENTO)) $criteria->add(Endereco1Peer::COMPLEMENTO, $this->complemento);
	...
	{
		$criteria = new Criteria(Endereco1Peer::DATABASE_NAME);
		$criteria->add(Endereco1Peer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Endereco1 (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Endereco1 Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     Endereco1Peer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new Endereco1Peer();
		}
		return self::$peer;
	}
	...
	public function getCidade(PropelPDO $con = null)
	{
		if ($this->aCidade === null && ($this->cidade_id !== null)) {
			$this->aCidade = CidadeQuery::create()->findPk($this->cidade_id, $con);
			/* The following can be used additionally to
				guarantee the related object contains a reference
				to this object.  This level of coupling may, however, be
				undesirable since it could result in an only partially populated collection
				in the referenced object.
				$this->aCidade->addEndereco1s($this);
			 */
		}
		return $this->aCidade;
	}
	 ...
	 * If this Endereco1 is new, it will return
	 ...
	 * Otherwise if this Endereco1 is new, it will return
	 * an empty collection; or if this Endereco1 has previously
	 * been saved, it will retrieve related Usuarios from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Endereco1.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	...
	{
		return (string) $this->exportTo(Endereco1Peer::DEFAULT_STRING_FORMAT);
	}
