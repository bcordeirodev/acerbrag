...
/**
 * Base class that represents a row from the 'fase_processo' table.
 *
 * 
 *
 * @package    propel.generator.Default.om
 */
abstract class BaseFaseProcesso extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'FaseProcessoPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        FaseProcessoPeer
	 */
	 ...
	 * @return     FaseProcesso The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = FaseProcessoPeer::ID;
		}
	...
	public function setNome($v)
	{
		...
		if ($this->nome !== $v) {
			$this->nome = $v;
			$this->modifiedColumns[] = FaseProcessoPeer::NOME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = FaseProcessoPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating FaseProcesso object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = FaseProcessoPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = FaseProcessoQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				FaseProcessoPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(FaseProcessoPeer::ID)) {
			$modifiedColumns[':p' . $index++]  = '`ID`';
		}
		if ($this->isColumnModified(FaseProcessoPeer::NOME)) {
			$modifiedColumns[':p' . $index++]  = '`NOME`';
		}

		$sql = sprintf(
			'INSERT INTO `fase_processo` (%s) VALUES (%s)',
	...
	{
		$pos = FaseProcessoPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['FaseProcesso'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['FaseProcesso'][$this->getPrimaryKey()] = true;
		$keys = FaseProcessoPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getNome(),
		);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FaseProcessoPeer::ID)) $criteria->add(FaseProcessoPeer::ID, $this->id);
		if ($this->isColumnModified(FaseProcessoPeer::NOME)) $criteria->add(FaseProcessoPeer::NOME, $this->nome);
	...
	{
		$criteria = new Criteria(FaseProcessoPeer::DATABASE_NAME);
		$criteria->add(FaseProcessoPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of FaseProcesso (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     FaseProcesso Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     FaseProcessoPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new FaseProcessoPeer();
		}
		return self::$peer;
	}
	 ...
	 * If this FaseProcesso is new, it will return
	 ...
	 * Otherwise if this FaseProcesso is new, it will return
	 * an empty collection; or if this FaseProcesso has previously
	 * been saved, it will retrieve related Processos from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in FaseProcesso.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	...
	{
		return (string) $this->exportTo(FaseProcessoPeer::DEFAULT_STRING_FORMAT);
	}
