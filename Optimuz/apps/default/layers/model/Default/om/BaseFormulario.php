...
abstract class BaseFormulario extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the coordernada field.
	 * @var        string
	 */
	protected $coordernada;
	...
	/**
	 * @var        array Gravacao[] Collection to store aggregation of Gravacao objects.
	 */
	protected $collGravacaos;
	 ...
	 * @var		array
	 */
	protected $gravacaosScheduledForDeletion = null;
	...
	/**
	 * Get the [coordernada] column value.
	 * 
	 * @return     string
	 */
	public function getCoordernada()
	{
		return $this->coordernada;
	}
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = FormularioPeer::ID;
		}
	...
	public function setUsuarioId($v)
	{
		...
		if ($this->usuario_id !== $v) {
			$this->usuario_id = $v;
			$this->modifiedColumns[] = FormularioPeer::USUARIO_ID;
		}
	...
	public function setPesquisaId($v)
	{
		...
		if ($this->pesquisa_id !== $v) {
			$this->pesquisa_id = $v;
			$this->modifiedColumns[] = FormularioPeer::PESQUISA_ID;
		}
	...
	public function setDataInicio($v)
	{
		...
		if ($this->data_inicio !== null || $dt !== null) {
			...
			if ($currentDateAsString !== $newDateAsString) {
				...
				$this->modifiedColumns[] = FormularioPeer::DATA_INICIO;
			}
	...
	public function setDataFim($v)
	{
		...
		if ($this->data_fim !== null || $dt !== null) {
			...
			if ($currentDateAsString !== $newDateAsString) {
				...
				$this->modifiedColumns[] = FormularioPeer::DATA_FIM;
			}
		} // if either are not null
	...
	/**
	 * Set the value of [coordernada] column.
	 * 
	 * @param      string $v new value
	 * @return     Formulario The current object (for fluent API support)
	 */
	public function setCoordernada($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->coordernada !== $v) {
			$this->coordernada = $v;
			$this->modifiedColumns[] = FormularioPeer::COORDERNADA;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->data_fim = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->coordernada = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->resetModified();
			...
			return $startcol + 6; // 6 = FormularioPeer::NUM_HYDRATE_COLUMNS.
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = FormularioQuery::create()
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->gravacaosScheduledForDeletion !== null) {
				if (!$this->gravacaosScheduledForDeletion->isEmpty()) {
					GravacaoQuery::create()
						->filterByPrimaryKeys($this->gravacaosScheduledForDeletion->getPrimaryKeys(false))
						->delete($con);
					$this->gravacaosScheduledForDeletion = null;
				}
			}

			if ($this->collGravacaos !== null) {
				foreach ($this->collGravacaos as $referrerFK) {
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(FormularioPeer::ID)) {
			$modifiedColumns[':p' . $index++]  = '`ID`';
		}
		if ($this->isColumnModified(FormularioPeer::USUARIO_ID)) {
			$modifiedColumns[':p' . $index++]  = '`USUARIO_ID`';
		}
		if ($this->isColumnModified(FormularioPeer::PESQUISA_ID)) {
			$modifiedColumns[':p' . $index++]  = '`PESQUISA_ID`';
		}
		if ($this->isColumnModified(FormularioPeer::DATA_INICIO)) {
			$modifiedColumns[':p' . $index++]  = '`DATA_INICIO`';
		}
		if ($this->isColumnModified(FormularioPeer::DATA_FIM)) {
			$modifiedColumns[':p' . $index++]  = '`DATA_FIM`';
		}
		if ($this->isColumnModified(FormularioPeer::COORDERNADA)) {
			$modifiedColumns[':p' . $index++]  = '`COORDERNADA`';
		}

		$sql = sprintf(
			'INSERT INTO `formulario` (%s) VALUES (%s)',
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					...
					case '`DATA_FIM`':
						...
						break;
					case '`COORDERNADA`':
						$stmt->bindValue($identifier, $this->coordernada, PDO::PARAM_STR);
						break;
				}
			}
			$stmt->execute();
	...
	{
		if (isset($alreadyDumpedObjects['Formulario'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Formulario'][$this->getPrimaryKey()] = true;
		...
		$result = array(
			...
			$keys[4] => $this->getDataFim(),
			$keys[5] => $this->getCoordernada(),
		);
		if ($includeForeignObjects) {
			...
			if (null !== $this->collGravacaos) {
				$result['Gravacaos'] = $this->collGravacaos->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
			}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[4], $arr)) $this->setDataFim($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setCoordernada($arr[$keys[5]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(FormularioPeer::ID)) $criteria->add(FormularioPeer::ID, $this->id);
		if ($this->isColumnModified(FormularioPeer::USUARIO_ID)) $criteria->add(FormularioPeer::USUARIO_ID, $this->usuario_id);
		if ($this->isColumnModified(FormularioPeer::PESQUISA_ID)) $criteria->add(FormularioPeer::PESQUISA_ID, $this->pesquisa_id);
		if ($this->isColumnModified(FormularioPeer::DATA_INICIO)) $criteria->add(FormularioPeer::DATA_INICIO, $this->data_inicio);
		if ($this->isColumnModified(FormularioPeer::DATA_FIM)) $criteria->add(FormularioPeer::DATA_FIM, $this->data_fim);
		if ($this->isColumnModified(FormularioPeer::COORDERNADA)) $criteria->add(FormularioPeer::COORDERNADA, $this->coordernada);
	...
	{
		$criteria = new Criteria(FormularioPeer::DATABASE_NAME);
		$criteria->add(FormularioPeer::ID, $this->id);
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setCoordernada($this->getCoordernada());
		...
		if ($deepCopy && !$this->startCopy) {
			...
			foreach ($this->getGravacaos() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addGravacao($relObj->copy($deepCopy));
				}
			}
	...
	public function getPesquisa(PropelPDO $con = null)
	{
		if ($this->aPesquisa === null && ($this->pesquisa_id !== null)) {
			...
			/* The following can be used additionally to
				...
				$this->aPesquisa->addFormularios($this);
			 */
		}
	...
	public function getUsuario(PropelPDO $con = null)
	{
		if ($this->aUsuario === null && ($this->usuario_id !== null)) {
			...
			/* The following can be used additionally to
				...
				undesirable since it could result in an only partially populated collection
				in the referenced object.
				$this->aUsuario->addFormularios($this);
			 */
		}
	...
	{
		if ('Gravacao' == $relationName) {
			return $this->initGravacaos();
		}
	...
	/**
	 * Clears out the collGravacaos collection
	 *
	 ...
	 * @see        addGravacaos()
	 */
	public function clearGravacaos()
	{
		$this->collGravacaos = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collGravacaos collection.
	 *
	 * By default this just sets the collGravacaos collection to an empty array (like clearcollGravacaos());
	 ...
	 * @return     void
	 */
	public function initGravacaos($overrideExisting = true)
	{
		if (null !== $this->collGravacaos && !$overrideExisting) {
			return;
		}
		$this->collGravacaos = new PropelObjectCollection();
		$this->collGravacaos->setModel('Gravacao');
	}

	/**
	 * Gets an array of Gravacao objects which contain a foreign key that references this object.
	 *
	 * If the $criteria is not null, it is used to always fetch the results from the database.
	 * Otherwise the results are fetched from the database the first time, then cached.
	 * Next time the same method is called without $criteria, the cached collection is returned.
	 * If this Formulario is new, it will return
	 * an empty collection or the current collection; the criteria is ignored on a new object.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	 * @return     PropelCollection|array Gravacao[] List of Gravacao objects
	 * @throws     PropelException
	 */
	public function getGravacaos($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collGravacaos || null !== $criteria) {
			if ($this->isNew() && null === $this->collGravacaos) {
				// return empty collection
				$this->initGravacaos();
			} else {
				$collGravacaos = GravacaoQuery::create(null, $criteria)
					->filterByFormulario($this)
					->find($con);
				if (null !== $criteria) {
					return $collGravacaos;
				}
				$this->collGravacaos = $collGravacaos;
			}
		}
		return $this->collGravacaos;
	}

	/**
	 * Sets a collection of Gravacao objects related by a one-to-many relationship
	 * to the current object.
	 * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
	 * and new objects from the given Propel collection.
	 *
	 * @param      PropelCollection $gravacaos A Propel collection.
	 * @param      PropelPDO $con Optional connection object
	 */
	public function setGravacaos(PropelCollection $gravacaos, PropelPDO $con = null)
	{
		$this->gravacaosScheduledForDeletion = $this->getGravacaos(new Criteria(), $con)->diff($gravacaos);

		foreach ($gravacaos as $gravacao) {
			// Fix issue with collection modified by reference
			if ($gravacao->isNew()) {
				$gravacao->setFormulario($this);
			}
			$this->addGravacao($gravacao);
		}

		$this->collGravacaos = $gravacaos;
	}

	/**
	 * Returns the number of related Gravacao objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related Gravacao objects.
	 * @throws     PropelException
	 */
	public function countGravacaos(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if(null === $this->collGravacaos || null !== $criteria) {
			if ($this->isNew() && null === $this->collGravacaos) {
				return 0;
			} else {
				$query = GravacaoQuery::create(null, $criteria);
				if($distinct) {
					$query->distinct();
				}
				return $query
			...
			return count($this->collGravacaos);
		}
	}

	/**
	 * Method called to associate a Gravacao object to this object
	 * through the Gravacao foreign key attribute.
	 *
	 * @param      Gravacao $l Gravacao
	 * @return     Formulario The current object (for fluent API support)
	 */
	public function addGravacao(Gravacao $l)
	{
		if ($this->collGravacaos === null) {
			$this->initGravacaos();
		}
		if (!$this->collGravacaos->contains($l)) { // only add it if the **same** object is not already associated
			$this->doAddGravacao($l);
		}
	...
	/**
	 * @param	Gravacao $gravacao The gravacao object to add.
	 */
	protected function doAddGravacao($gravacao)
	{
		$this->collGravacaos[]= $gravacao;
		$gravacao->setFormulario($this);
	}
	...
	{
		if ($deep) {
			if ($this->collGravacaos) {
				foreach ($this->collGravacaos as $o) {
		...
		if ($this->collGravacaos instanceof PropelCollection) {
			$this->collGravacaos->clearIterator();
		}
		$this->collGravacaos = null;
	...
	{
		return (string) $this->exportTo(FormularioPeer::DEFAULT_STRING_FORMAT);
	}
