...
abstract class BaseForum extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the topico field.
	 * @var        string
	 */
	protected $topico;

	/**
	 * The value for the data_cricacao field.
	 * @var        string
	 */
	protected $data_cricacao;
	...
	/**
	 * Get the [optionally formatted] temporal [data_cricacao] column value.
	 * 
	 *
	 ...
	 */
	public function getDataCricacao($format = 'Y-m-d H:i:s')
	{
		if ($this->data_cricacao === null) {
			return null;
		}


		if ($this->data_cricacao === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->data_cricacao);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->data_cricacao, true), $x);
			}
		}
	...
	public function setUsuarioId($v)
	{
		...
		if ($this->usuario_id !== $v) {
			$this->usuario_id = $v;
			$this->modifiedColumns[] = ForumPeer::USUARIO_ID;
		}
	...
	public function setTitulo($v)
	{
		...
		if ($this->titulo !== $v) {
			$this->titulo = $v;
			$this->modifiedColumns[] = ForumPeer::TITULO;
		}
	...
	/**
	 * Set the value of [topico] column.
	 * 
	 * @param      string $v new value
	 * @return     Forum The current object (for fluent API support)
	 */
	public function setTopico($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}
		...
		if ($this->topico !== $v) {
			$this->topico = $v;
			$this->modifiedColumns[] = ForumPeer::TOPICO;
		}
	...
	/**
	 * Sets the value of [data_cricacao] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.
	 *               Empty strings are treated as NULL.
	 * @return     Forum The current object (for fluent API support)
	 */
	public function setDataCricacao($v)
	{
		$dt = PropelDateTime::newInstance($v, null, 'DateTime');
		if ($this->data_cricacao !== null || $dt !== null) {
			$currentDateAsString = ($this->data_cricacao !== null && $tmpDt = new DateTime($this->data_cricacao)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newDateAsString = $dt ? $dt->format('Y-m-d H:i:s') : null;
			if ($currentDateAsString !== $newDateAsString) {
				$this->data_cricacao = $newDateAsString;
				$this->modifiedColumns[] = ForumPeer::DATA_CRICACAO;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->topico = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->data_cricacao = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->resetModified();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(ForumPeer::USUARIO_ID)) {
			$modifiedColumns[':p' . $index++]  = '`USUARIO_ID`';
		}
		if ($this->isColumnModified(ForumPeer::TITULO)) {
			$modifiedColumns[':p' . $index++]  = '`TITULO`';
		}
		if ($this->isColumnModified(ForumPeer::TOPICO)) {
			$modifiedColumns[':p' . $index++]  = '`TOPICO`';
		}
		if ($this->isColumnModified(ForumPeer::DATA_CRICACAO)) {
			$modifiedColumns[':p' . $index++]  = '`DATA_CRICACAO`';
		}
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					...
					case '`TITULO`':						
						...
						break;
					case '`TOPICO`':						
						$stmt->bindValue($identifier, $this->topico, PDO::PARAM_STR);
						break;
					case '`DATA_CRICACAO`':						
						$stmt->bindValue($identifier, $this->data_cricacao, PDO::PARAM_STR);
						break;
				}
			}
			$stmt->execute();
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			...
			case 2:
				...
				break;
			case 3:
				return $this->getTopico();
				break;
			case 4:
				return $this->getDataCricacao();
				break;
			default:
				return null;
				break;
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
	{
		...
		$result = array(
			...
			$keys[3] => $this->getTopico(),
			$keys[4] => $this->getDataCricacao(),
		);
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			...
			case 2:
				...
				break;
			case 3:
				$this->setTopico($value);
				break;
			case 4:
				$this->setDataCricacao($value);
				break;
		} // switch()
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setTopico($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setDataCricacao($arr[$keys[4]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ForumPeer::USUARIO_ID)) $criteria->add(ForumPeer::USUARIO_ID, $this->usuario_id);
		if ($this->isColumnModified(ForumPeer::TITULO)) $criteria->add(ForumPeer::TITULO, $this->titulo);
		if ($this->isColumnModified(ForumPeer::TOPICO)) $criteria->add(ForumPeer::TOPICO, $this->topico);
		if ($this->isColumnModified(ForumPeer::DATA_CRICACAO)) $criteria->add(ForumPeer::DATA_CRICACAO, $this->data_cricacao);
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setTopico($this->getTopico());
		$copyObj->setDataCricacao($this->getDataCricacao());
	 ...
	 * @return     PropelCollection|array CurtidaForum[] List of CurtidaForum objects
	 */
	public function getCurtidaForumsJoinUsuario($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = CurtidaForumQuery::create(null, $criteria);
	 ...
	 * @return     PropelCollection|array RespostaForum[] List of RespostaForum objects
	 */
	public function getRespostaForumsJoinUsuario($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = RespostaForumQuery::create(null, $criteria);
		$query->joinWith('Usuario', $join_behavior);
