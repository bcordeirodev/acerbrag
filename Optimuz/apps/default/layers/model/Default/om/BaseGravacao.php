...
/**
 * Base class that represents a row from the 'gravacao' table.
 *
 * 
 *
 * @package    propel.generator.Default.om
 */
abstract class BaseGravacao extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'GravacaoPeer';
	 ...
	 * @return     Gravacao The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = GravacaoPeer::ID;
		}
	...
	public function setColetaPesquisaId($v)
	{
		...
		if ($this->coleta_pesquisa_id !== $v) {
			...
			$this->modifiedColumns[] = GravacaoPeer::COLETA_PESQUISA_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = GravacaoPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating Gravacao object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = GravacaoPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = GravacaoQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				GravacaoPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(GravacaoPeer::ID)) {
			$modifiedColumns[':p' . $index++]  = '`ID`';
		}
		if ($this->isColumnModified(GravacaoPeer::COLETA_PESQUISA_ID)) {
			$modifiedColumns[':p' . $index++]  = '`COLETA_PESQUISA_ID`';
		}

		$sql = sprintf(
			'INSERT INTO `gravacao` (%s) VALUES (%s)',
	...
	{
		$pos = GravacaoPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['Gravacao'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Gravacao'][$this->getPrimaryKey()] = true;
		$keys = GravacaoPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(GravacaoPeer::ID)) $criteria->add(GravacaoPeer::ID, $this->id);
		if ($this->isColumnModified(GravacaoPeer::COLETA_PESQUISA_ID)) $criteria->add(GravacaoPeer::COLETA_PESQUISA_ID, $this->coleta_pesquisa_id);
	...
	{
		$criteria = new Criteria(GravacaoPeer::DATABASE_NAME);
		$criteria->add(GravacaoPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Gravacao (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Gravacao Clone of current object.
	 * @throws     PropelException
	 */
	...
	public function getColetaPesquisa(PropelPDO $con = null)
	{
		if ($this->aColetaPesquisa === null && ($this->coleta_pesquisa_id !== null)) {
			...
			/* The following can be used additionally to
				...
				$this->aColetaPesquisa->addGravacaos($this);
			 */
		}
	...
	{
		return (string) $this->exportTo(GravacaoPeer::DEFAULT_STRING_FORMAT);
	}
