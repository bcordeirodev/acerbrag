...
abstract class BaseNoticia extends BaseObject  implements Persistent
{
	...
	public function setUsuarioId($v)
	{
		...
		if ($this->usuario_id !== $v) {
			$this->usuario_id = $v;
			$this->modifiedColumns[] = NoticiaPeer::USUARIO_ID;
		}
	...
	public function setTema($v)
	{
		...
		if ($this->tema !== $v) {
			$this->tema = $v;
			$this->modifiedColumns[] = NoticiaPeer::TEMA;
		}
	...
	public function setSubTitulo($v)
	{
		...
		if ($this->sub_titulo !== $v) {
			$this->sub_titulo = $v;
			$this->modifiedColumns[] = NoticiaPeer::SUB_TITULO;
		}
	...
	public function setDescricao($v)
	{
		...
		if ($this->descricao !== $v) {
			$this->descricao = $v;
			$this->modifiedColumns[] = NoticiaPeer::DESCRICAO;
		}
	...
	public function setDataCadastro($v)
	{
		...
		if ($this->data_cadastro !== null || $dt !== null) {
			...
			if ($currentDateAsString !== $newDateAsString) {
				...
				$this->modifiedColumns[] = NoticiaPeer::DATA_CADASTRO;
			}
	...
	public function setVisualizacao($v)
	{
		...
		if ($this->visualizacao !== $v) {
			$this->visualizacao = $v;
			$this->modifiedColumns[] = NoticiaPeer::VISUALIZACAO;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->tema = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->titulo = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->sub_titulo = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = NoticiaQuery::create()
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(NoticiaPeer::ID)) {
			$modifiedColumns[':p' . $index++]  = '`ID`';
		}
		if ($this->isColumnModified(NoticiaPeer::USUARIO_ID)) {
			$modifiedColumns[':p' . $index++]  = '`USUARIO_ID`';
		}
		if ($this->isColumnModified(NoticiaPeer::TEMA)) {
			$modifiedColumns[':p' . $index++]  = '`TEMA`';
		}
		if ($this->isColumnModified(NoticiaPeer::TITULO)) {
			$modifiedColumns[':p' . $index++]  = '`TITULO`';
		}
		if ($this->isColumnModified(NoticiaPeer::SUB_TITULO)) {
			$modifiedColumns[':p' . $index++]  = '`SUB_TITULO`';
		}
		if ($this->isColumnModified(NoticiaPeer::DESCRICAO)) {
			$modifiedColumns[':p' . $index++]  = '`DESCRICAO`';
		}
		if ($this->isColumnModified(NoticiaPeer::DATA_CADASTRO)) {
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					...
					case '`TITULO`':						
						...
						break;
					case '`SUB_TITULO`':						
						$stmt->bindValue($identifier, $this->sub_titulo, PDO::PARAM_STR);
						break;
					case '`DESCRICAO`':						
	...
	{
		if (isset($alreadyDumpedObjects['Noticia'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Noticia'][$this->getPrimaryKey()] = true;
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[2], $arr)) $this->setTema($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setTitulo($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setSubTitulo($arr[$keys[4]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(NoticiaPeer::USUARIO_ID)) $criteria->add(NoticiaPeer::USUARIO_ID, $this->usuario_id);
		if ($this->isColumnModified(NoticiaPeer::TEMA)) $criteria->add(NoticiaPeer::TEMA, $this->tema);
		if ($this->isColumnModified(NoticiaPeer::TITULO)) $criteria->add(NoticiaPeer::TITULO, $this->titulo);
		if ($this->isColumnModified(NoticiaPeer::SUB_TITULO)) $criteria->add(NoticiaPeer::SUB_TITULO, $this->sub_titulo);
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setSubTitulo($this->getSubTitulo());
	...
	public function getUsuario(PropelPDO $con = null)
	{
		if ($this->aUsuario === null && ($this->usuario_id !== null)) {
			...
			/* The following can be used additionally to
				...
				$this->aUsuario->addNoticias($this);
			 */
		}
	 ...
	 * If this Noticia is new, it will return
	...
	public function setComentarioNoticias(PropelCollection $comentarioNoticias, PropelPDO $con = null)
	{
		...
		foreach ($comentarioNoticias as $comentarioNoticia) {
			...
			if ($comentarioNoticia->isNew()) {
				$comentarioNoticia->setNoticia($this);
			}
	 ...
	 * Otherwise if this Noticia is new, it will return
	 * an empty collection; or if this Noticia has previously
	 * been saved, it will retrieve related ComentarioNoticias from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Noticia.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	 ...
	 * @return     PropelCollection|array ComentarioNoticia[] List of ComentarioNoticia objects
	 */
	public function getComentarioNoticiasJoinUsuario($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = ComentarioNoticiaQuery::create(null, $criteria);
	...
	{
		return (string) $this->exportTo(NoticiaPeer::DEFAULT_STRING_FORMAT);
	}
