...
/**
 * Base class that represents a row from the 'titulo' table.
 *
 * 
 *
 * @package    propel.generator.Default.om
 */
abstract class BaseTitulo extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'TituloPeer';
	 ...
	 * @return     Titulo The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = TituloPeer::ID;
		}
	...
	public function setAdvogadoId($v)
	{
		...
		if ($this->advogado_id !== $v) {
			$this->advogado_id = $v;
			$this->modifiedColumns[] = TituloPeer::ADVOGADO_ID;
		}
	...
	public function setDescricao($v)
	{
		...
		if ($this->descricao !== $v) {
			$this->descricao = $v;
			$this->modifiedColumns[] = TituloPeer::DESCRICAO;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 3; // 3 = TituloPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating Titulo object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = TituloPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = TituloQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				TituloPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(TituloPeer::ID)) {
			$modifiedColumns[':p' . $index++]  = '`ID`';
		}
		if ($this->isColumnModified(TituloPeer::ADVOGADO_ID)) {
			$modifiedColumns[':p' . $index++]  = '`ADVOGADO_ID`';
		}
		if ($this->isColumnModified(TituloPeer::DESCRICAO)) {
			$modifiedColumns[':p' . $index++]  = '`DESCRICAO`';
		}

		$sql = sprintf(
			'INSERT INTO `titulo` (%s) VALUES (%s)',
	...
	{
		$pos = TituloPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['Titulo'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Titulo'][$this->getPrimaryKey()] = true;
		$keys = TituloPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(TituloPeer::ID)) $criteria->add(TituloPeer::ID, $this->id);
		if ($this->isColumnModified(TituloPeer::ADVOGADO_ID)) $criteria->add(TituloPeer::ADVOGADO_ID, $this->advogado_id);
		if ($this->isColumnModified(TituloPeer::DESCRICAO)) $criteria->add(TituloPeer::DESCRICAO, $this->descricao);
	...
	{
		$criteria = new Criteria(TituloPeer::DATABASE_NAME);
		$criteria->add(TituloPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Titulo (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Titulo Clone of current object.
	 * @throws     PropelException
	 */
	...
	public function getAdvogado(PropelPDO $con = null)
	{
		if ($this->aAdvogado === null && ($this->advogado_id !== null)) {
			...
			/* The following can be used additionally to
				...
				$this->aAdvogado->addTitulos($this);
			 */
		}
	...
	{
		return (string) $this->exportTo(TituloPeer::DEFAULT_STRING_FORMAT);
	}
