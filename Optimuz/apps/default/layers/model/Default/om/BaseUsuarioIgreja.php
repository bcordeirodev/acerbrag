...
/**
 * Base class that represents a row from the 'usuario_igreja' table.
 *
 * 
 *
 * @package    propel.generator.Default.om
 */
abstract class BaseUsuarioIgreja extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'UsuarioIgrejaPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        UsuarioIgrejaPeer
	 */
	 ...
	 * @return     UsuarioIgreja The current object (for fluent API support)
	 */
	public function setUsuarioId($v)
	{
		...
		if ($this->usuario_id !== $v) {
			$this->usuario_id = $v;
			$this->modifiedColumns[] = UsuarioIgrejaPeer::USUARIO_ID;
		}
	...
	public function setIgrejaId($v)
	{
		...
		if ($this->igreja_id !== $v) {
			$this->igreja_id = $v;
			$this->modifiedColumns[] = UsuarioIgrejaPeer::IGREJA_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = UsuarioIgrejaPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating UsuarioIgreja object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = UsuarioIgrejaPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = UsuarioIgrejaQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				UsuarioIgrejaPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(UsuarioIgrejaPeer::USUARIO_ID)) {
			$modifiedColumns[':p' . $index++]  = '`USUARIO_ID`';
		}
		if ($this->isColumnModified(UsuarioIgrejaPeer::IGREJA_ID)) {
			$modifiedColumns[':p' . $index++]  = '`IGREJA_ID`';
		}

		$sql = sprintf(
			'INSERT INTO `usuario_igreja` (%s) VALUES (%s)',
	...
	{
		$pos = UsuarioIgrejaPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['UsuarioIgreja'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['UsuarioIgreja'][serialize($this->getPrimaryKey())] = true;
		$keys = UsuarioIgrejaPeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(UsuarioIgrejaPeer::USUARIO_ID)) $criteria->add(UsuarioIgrejaPeer::USUARIO_ID, $this->usuario_id);
		if ($this->isColumnModified(UsuarioIgrejaPeer::IGREJA_ID)) $criteria->add(UsuarioIgrejaPeer::IGREJA_ID, $this->igreja_id);
	...
	{
		$criteria = new Criteria(UsuarioIgrejaPeer::DATABASE_NAME);
		$criteria->add(UsuarioIgrejaPeer::USUARIO_ID, $this->usuario_id);
		$criteria->add(UsuarioIgrejaPeer::IGREJA_ID, $this->igreja_id);
	...
	{
		return (null === $this->getUsuarioId()) && (null === $this->getIgrejaId());
	}
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of UsuarioIgreja (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     UsuarioIgreja Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     UsuarioIgrejaPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new UsuarioIgrejaPeer();
		}
		return self::$peer;
	}
	...
	public function getUsuario(PropelPDO $con = null)
	{
		if ($this->aUsuario === null && ($this->usuario_id !== null)) {
			...
			/* The following can be used additionally to
				...
				$this->aUsuario->addUsuarioIgrejas($this);
			 */
		}
	...
	public function getIgreja(PropelPDO $con = null)
	{
		if ($this->aIgreja === null && ($this->igreja_id !== null)) {
			...
			/* The following can be used additionally to
				...
				undesirable since it could result in an only partially populated collection
				in the referenced object.
				$this->aIgreja->addUsuarioIgrejas($this);
			 */
		}
		return $this->aIgreja;
	}
	...
	{
		return (string) $this->exportTo(UsuarioIgrejaPeer::DEFAULT_STRING_FORMAT);
	}
