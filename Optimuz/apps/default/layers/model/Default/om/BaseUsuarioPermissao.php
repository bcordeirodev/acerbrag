...
/**
 * Base class that represents a row from the 'usuario_permissao' table.
 *
 * 
 *
 * @package    propel.generator.Default.om
 */
abstract class BaseUsuarioPermissao extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'UsuarioPermissaoPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        UsuarioPermissaoPeer
	 */
	 ...
	 * @return     UsuarioPermissao The current object (for fluent API support)
	 */
	public function setUsuarioId($v)
	{
		...
		if ($this->usuario_id !== $v) {
			$this->usuario_id = $v;
			$this->modifiedColumns[] = UsuarioPermissaoPeer::USUARIO_ID;
		}
	...
	public function setPermissaoId($v)
	{
		...
		if ($this->permissao_id !== $v) {
			$this->permissao_id = $v;
			$this->modifiedColumns[] = UsuarioPermissaoPeer::PERMISSAO_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = UsuarioPermissaoPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating UsuarioPermissao object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = UsuarioPermissaoPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = UsuarioPermissaoQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				UsuarioPermissaoPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(UsuarioPermissaoPeer::USUARIO_ID)) {
			$modifiedColumns[':p' . $index++]  = '`USUARIO_ID`';
		}
		if ($this->isColumnModified(UsuarioPermissaoPeer::PERMISSAO_ID)) {
			$modifiedColumns[':p' . $index++]  = '`PERMISSAO_ID`';
		}

		$sql = sprintf(
			'INSERT INTO `usuario_permissao` (%s) VALUES (%s)',
	...
	{
		$pos = UsuarioPermissaoPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['UsuarioPermissao'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['UsuarioPermissao'][serialize($this->getPrimaryKey())] = true;
		$keys = UsuarioPermissaoPeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(UsuarioPermissaoPeer::USUARIO_ID)) $criteria->add(UsuarioPermissaoPeer::USUARIO_ID, $this->usuario_id);
		if ($this->isColumnModified(UsuarioPermissaoPeer::PERMISSAO_ID)) $criteria->add(UsuarioPermissaoPeer::PERMISSAO_ID, $this->permissao_id);
	...
	{
		$criteria = new Criteria(UsuarioPermissaoPeer::DATABASE_NAME);
		$criteria->add(UsuarioPermissaoPeer::USUARIO_ID, $this->usuario_id);
		$criteria->add(UsuarioPermissaoPeer::PERMISSAO_ID, $this->permissao_id);
	...
	{
		return (null === $this->getUsuarioId()) && (null === $this->getPermissaoId());
	}
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of UsuarioPermissao (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     UsuarioPermissao Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     UsuarioPermissaoPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new UsuarioPermissaoPeer();
		}
		return self::$peer;
	}
	...
	public function getUsuario(PropelPDO $con = null)
	{
		if ($this->aUsuario === null && ($this->usuario_id !== null)) {
			...
			/* The following can be used additionally to
				...
				$this->aUsuario->addUsuarioPermissaos($this);
			 */
		}
	...
	public function getPermissao(PropelPDO $con = null)
	{
		if ($this->aPermissao === null && ($this->permissao_id !== null)) {
			...
			/* The following can be used additionally to
				...
				undesirable since it could result in an only partially populated collection
				in the referenced object.
				$this->aPermissao->addUsuarioPermissaos($this);
			 */
		}
	...
	{
		return (string) $this->exportTo(UsuarioPermissaoPeer::DEFAULT_STRING_FORMAT);
	}
