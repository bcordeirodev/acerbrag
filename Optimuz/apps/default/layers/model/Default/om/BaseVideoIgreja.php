...
/**
 * Base class that represents a row from the 'video_igreja' table.
 *
 * 
 *
 * @package    propel.generator.Default.om
 */
abstract class BaseVideoIgreja extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'VideoIgrejaPeer';
	 ...
	 * @return     VideoIgreja The current object (for fluent API support)
	 */
	public function setVideoId($v)
	{
		...
		if ($this->video_id !== $v) {
			$this->video_id = $v;
			$this->modifiedColumns[] = VideoIgrejaPeer::VIDEO_ID;
		}
	...
	public function setIgrejaId($v)
	{
		...
		if ($this->igreja_id !== $v) {
			$this->igreja_id = $v;
			$this->modifiedColumns[] = VideoIgrejaPeer::IGREJA_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->video_id = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			...
			return $startcol + 2; // 2 = VideoIgrejaPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating VideoIgreja object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = VideoIgrejaPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = VideoIgrejaQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				VideoIgrejaPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(VideoIgrejaPeer::VIDEO_ID)) {
			$modifiedColumns[':p' . $index++]  = '`VIDEO_ID`';
		}
		if ($this->isColumnModified(VideoIgrejaPeer::IGREJA_ID)) {
			$modifiedColumns[':p' . $index++]  = '`IGREJA_ID`';
		}

		$sql = sprintf(
			'INSERT INTO `video_igreja` (%s) VALUES (%s)',
	...
	{
		$pos = VideoIgrejaPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['VideoIgreja'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['VideoIgreja'][serialize($this->getPrimaryKey())] = true;
		$keys = VideoIgrejaPeer::getFieldNames($keyType);
		$result = array(
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[0], $arr)) $this->setVideoId($arr[$keys[0]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(VideoIgrejaPeer::VIDEO_ID)) $criteria->add(VideoIgrejaPeer::VIDEO_ID, $this->video_id);
		if ($this->isColumnModified(VideoIgrejaPeer::IGREJA_ID)) $criteria->add(VideoIgrejaPeer::IGREJA_ID, $this->igreja_id);
	...
	{
		$criteria = new Criteria(VideoIgrejaPeer::DATABASE_NAME);
		$criteria->add(VideoIgrejaPeer::VIDEO_ID, $this->video_id);
		$criteria->add(VideoIgrejaPeer::IGREJA_ID, $this->igreja_id);
	...
	{
		return (null === $this->getVideoId()) && (null === $this->getIgrejaId());
	}
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of VideoIgreja (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     VideoIgreja Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     VideoIgrejaPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new VideoIgrejaPeer();
		}
		return self::$peer;
	}
	...
	public function getIgreja(PropelPDO $con = null)
	{
		if ($this->aIgreja === null && ($this->igreja_id !== null)) {
			...
			/* The following can be used additionally to
				...
				$this->aIgreja->addVideoIgrejas($this);
			 */
		}
		return $this->aIgreja;
	}
	...
	public function getVideo(PropelPDO $con = null)
	{
		if ($this->aVideo === null && ($this->video_id !== null)) {
			...
			/* The following can be used additionally to
				...
				undesirable since it could result in an only partially populated collection
				in the referenced object.
				$this->aVideo->addVideoIgrejas($this);
			 */
		}
		return $this->aVideo;
	}
	...
	{
		return (string) $this->exportTo(VideoIgrejaPeer::DEFAULT_STRING_FORMAT);
	}
