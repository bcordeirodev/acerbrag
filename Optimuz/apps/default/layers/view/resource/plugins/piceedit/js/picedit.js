	...
	Plugin.prototype = {
		...
		set_loading: function (message) {
			if(message && message == 1) {
				return this.set_messagebox("Processando...", false, false);
			}
			else return this.set_messagebox("Por favor aguarde...", false, false);
		},
		...
		camera_open: function() {
			...
			getUserMedia({
				...
				function(err) {
					return _this.set_messagebox("Nenhuma camera detectada! Por favor permita o acesso a camera!");
				}
			);
		},
		...
		_hideAllNav: function (message) {
			if(message && message == 1) {
				this.set_messagebox("Abra sua camera ou utilize sua camera para tirar uma foto!");
			}
		...
		_formsubmit: function() {
			if(!window.FormData) this.set_messagebox("Desculpe, seu navegador precisa ser atualizado!");
			else {
				var _this = this;
				this.set_loading().delay(200).promise().done(function() {
					_this._theformdata = new FormData();
					if(_this._image) {
					...
					_this._theformdata.append("id",$("#id").val());
					//send request
                    ...
                    request.onprogress = function(e) {
                        ...
                        _this.set_messagebox("Aguarde... Enviando... " + progress + "% Enviado.", false, false);
                    };
					request.open("post", "/filiado/upload-edit", true);
					request.onload = function(e) {
						if(this.status != 200) {
                            _this.set_messagebox("Servidor recusou o envio!");
                        }
                        else {
						    ...
						    else _this.set_messagebox("Imagem enviada com sucesso!");
                        }
